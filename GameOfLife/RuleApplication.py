...
class RuleApplication:
    ...
    def apply_rule(automaton, rule: Rules, rule_idx=0, offset=(1, 0), carry_over=True):
        ...
        if rule == Rules.ELEMENTARY:
            return RuleApplication.apply_elemantary_rule(automaton, rule_idx)
        ...
        for i in range(automaton.cols):
            for j in range(automaton.rows):
                if (j == 0 and y_off < 0) or\
                        ...
                        and not carry_over:
                    new_cells[i][(j + y_off) % automaton.rows] = 0
                    continue
                if (i == 0 and x_off < 0) or\
                        (i == automaton.cols - 1 and x_off > 0)\
                        and not carry_over:
                    new_cells[(i + x_off) % automaton.cols][j] = 0
                    continue
                new_cells[(i + x_off) % automaton.cols][(j + y_off) % automaton.rows] = automaton.cells[i][j]
        return new_cells
    ...
    @staticmethod
    def apply_elemantary_rule(automaton, rule_idx):
        new_cells = RuleApplication.apply_offset_rule(automaton, (0, 1), carry_over=False)
        rule = bin(rule_idx)
        print(rule)
        new_cells[0] = np.zeros(automaton.cols, dtype=int)
        #for j in range(automaton.cols):
         #   neighbor_pattern = automaton.get_row_neighbors(0, j)
        return new_cells
